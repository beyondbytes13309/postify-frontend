...
export default forwardRef(function Modal({visibility=true, setVisibility}, ref) {
    ...
    return (
        <>
            ...
            {visibility && <div className={styles.modalOverlay}>
                <div className={`${styles.wrapper} ${styles[variants[localConfig.variant] || 'alertModal']}`}>
                    ...
                    {localConfig.variant == 'warning' ? (
                        <div className={styles.buttons}>
                            <Button variant="destructive" onClick={() => handleButtonClick(0)}>{localConfig.buttonTexts[0] || "Yes"}</Button>
                            <Button variant="secondary" onClick={() => handleButtonClick(1)}>{localConfig.buttonTexts[1] || "Cancel"}</Button>
                        </div>   
                    ) : localConfig.variant == 'alert' ? (
                        <div  className={styles.buttons}>
                            <Button variant="success" onClick={() => handleButtonClick(0)}>{localConfig.buttonTexts[0] || "Okay" }</Button>
                        </div>
                    ) : localConfig.variant == 'input' ? (
                        <div className={styles.miniWrapper}>
                            <input type="text" onChange={(e) => localConfig.setModalInput(e.target.value)} value={localConfig.modalInput}/>
                            <div>
                                <Button variant="primary" onClick={() => handleButtonClick(0)}>{localConfig.buttonTexts[0] || "Done"}</Button>
                                <Button variant="secondary" onClick={() => handleButtonClick(1)}>{localConfig.buttonTexts[1] || "Cancel"}</Button>
                            </div>
